    ...
    <div class="p-6 md:p-8 space-y-8 max-w-7xl mx-auto">
      ...
      <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div class="flex items-center space-x-3">
          ...
          <a
            [routerLink]="['/org/investigation', defectCaseId()]"
      ...
      <div *ngIf="defectCase()" class="glass-card p-6 rounded-2xl">
        <div class="flex items-center space-x-4">
          <div
            class="w-12 h-12 rounded-xl bg-brand-100 dark:bg-brand-950/60 text-brand-600 dark:text-brand-400 flex items-center justify-center"
          >
            <span class="material-symbols-outlined text-2xl">inventory_2</span>
          </div>
          <div>
            <h2 class="text-lg font-bold text-slate-900 dark:text-white">
              {{ defectCase().productName || 'Unknown Product' }}
            </h2>
            <p class="text-xs text-slate-500">
              Serial: {{ defectCase().productSerialNumber }} | Batch: {{ defectCase().batchNumber }}
            </p>
          </div>
        </div>
      </div>
      ...
      <div class="glass-card rounded-2xl overflow-hidden">
        <div class="p-6 border-b border-slate-200 dark:border-slate-700">
          ...
          <p class="text-xs text-slate-500">
            Trace the product journey through each lifecycle stage
          </p>
        </div>
        ...
        <div *ngIf="!loading() && timeline().length > 0" class="p-6">
          <div class="relative">
            ...
            <div class="space-y-8">
              <div
                *ngFor="let stage of timeline(); let i = index"
              ...
              >
                ...
                <div class="relative z-10 flex-shrink-0">
                  <div
                    [class]="
                      ...
                      getStageColorClass(stage.stage)
                    "
                  >
                  ...
                  <div
                    *ngIf="animating() && i === currentStageIndex()"
                    class="absolute -inset-2 rounded-2xl bg-brand-400/30 animate-ping"
                  ></div>
                </div>
                ...
                <div class="flex-1 glass-card p-5 rounded-2xl space-y-3">
                  ...
                  <!-- Documents -->
                  <div
                    *ngIf="stage.documents && stage.documents.length > 0"
                  ...
                  >
                    ...
                    <div class="flex flex-wrap gap-2">
                      <span
                        *ngFor="let doc of stage.documents"
                        class="px-2 py-1 bg-blue-100 dark:bg-blue-950/50 text-blue-700 dark:text-blue-300 rounded-lg text-xs"
                      >
                        {{ doc }}
                      </span>
                    </div>
                  ...
                  <!-- Certificates -->
                  <div
                    *ngIf="stage.certificates && stage.certificates.length > 0"
                    class="pt-3 border-t border-slate-200 dark:border-slate-700"
                  >
                    <p class="text-[11px] font-bold uppercase text-slate-400 mb-2">Certificates</p>
                    <div class="flex flex-wrap gap-2">
                      <span
                        *ngFor="let cert of stage.certificates"
                        class="px-2 py-1 bg-green-100 dark:bg-green-950/50 text-green-700 dark:text-green-300 rounded-lg text-xs"
                      >
                        {{ cert }}
                      </span>
                    </div>
      ...
      <div *ngIf="animating()" class="glass-card p-6 rounded-2xl">
        <div class="flex items-center justify-between mb-3">
          ...
          <span class="text-xs text-slate-500"
            >{{ currentStageIndex() + 1 }} / {{ timeline().length }}</span
          >
        </div>
        <div class="w-full h-2 bg-slate-200 dark:bg-slate-700 rounded-full overflow-hidden">
          <div
            class="h-full bg-brand-600 transition-all duration-500"
            [style.width.%]="((currentStageIndex() + 1) / timeline().length) * 100"
          ></div>
        </div>
        <p class="text-xs text-slate-500 mt-2">
          Analyzing stage: {{ timeline()[currentStageIndex()]?.stage }}
        </p>
      </div>
...
export class OrgBacktrackingComponent implements OnInit {
  ...
  ngOnInit() {
    const id = this.route.snapshot.paramMap.get('defectCaseId');
    if (id) {
  ...
  loadBacktrackingData() {
    ...
    this.http.get<any>(`${this.apiUrl}/defects/${this.defectCaseId()}/backtracking`).subscribe({
      next: (res) => {
    ...
    this.http.get<any>(`${this.apiUrl}/defects/${this.defectCaseId()}`).subscribe({
      next: (res) => this.defectCase.set(res.success ? res.data : null),
      error: () => this.defectCase.set(null),
    });
  }
