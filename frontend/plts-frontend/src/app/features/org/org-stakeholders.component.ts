...
@Component({
  ...
  template: `
    <div class="p-6 md:p-8 space-y-8 max-w-7xl mx-auto">
      ...
      <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p class="text-xs text-slate-500 dark:text-slate-400 mt-1">
            Manage Manufacturers, Quality Assurance, Packaging & Transport, and Retailers for your organization.
          </p>
        </div>
      ...
      <!-- Role Tabs -->
      <div class="flex flex-wrap gap-2 p-1.5 bg-slate-100 dark:bg-slate-850 rounded-2xl w-full sm:w-fit text-xs font-bold">
        <button 
        ...
        <div class="overflow-x-auto">
          <table class="w-full text-left text-xs">
            ...
            <tbody class="divide-y divide-slate-100 dark:divide-slate-800 font-medium text-slate-700 dark:text-slate-300">
              <tr *ngIf="stakeholders().length === 0">
                <td colspan="7" class="py-8 text-center text-slate-400">
                  No stakeholders registered under this category yet. Click "Add Stakeholder" to create one.
                </td>
              </tr>
              <tr *ngFor="let s of stakeholders()" class="hover:bg-slate-50 dark:hover:bg-slate-800/40 transition-colors">
                ...
                <td class="py-4 px-6">
                  <span class="px-2.5 py-0.5 rounded-full text-[10px] font-extrabold uppercase bg-brand-50 text-brand-700 dark:bg-brand-950 dark:text-brand-300">
                    {{ s.role }}
                  </span>
                </td>
    ...
    <div *ngIf="showModal()" class="fixed inset-0 bg-slate-950/60 backdrop-blur-sm flex items-center justify-center p-4 z-50 animate-in fade-in">
      <div class="bg-white dark:bg-slate-900 rounded-3xl p-6 sm:p-8 max-w-lg w-full border border-slate-200 dark:border-slate-800 shadow-2xl space-y-6">
        ...
        <div class="flex items-center justify-between border-b border-slate-100 dark:border-slate-800 pb-4">
          <div>
            <h3 class="text-xl font-bold font-heading text-slate-900 dark:text-white">Add Stakeholder</h3>
            <p class="text-xs text-slate-400">System will automatically generate a non-editable User ID (e.g. MAN-000001)</p>
          </div>
        ...
        <form (ngSubmit)="saveStakeholder()" class="space-y-4">
          <div>
            <label class="block text-xs font-bold text-slate-700 dark:text-slate-300 mb-1">Stakeholder Role *</label>
            <select [(ngModel)]="form.role" name="role" required class="w-full px-3.5 py-2 bg-slate-50 dark:bg-slate-800 text-slate-900 dark:text-white rounded-xl border border-slate-200 dark:border-slate-700 text-xs font-bold focus:ring-2 focus:ring-brand-500/50 focus:outline-none">
          ...
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              ...
              <input type="text" [(ngModel)]="form.companyName" name="companyName" required placeholder="Apex Manufacturing Ltd" class="w-full px-3.5 py-2 bg-slate-50 dark:bg-slate-800 text-slate-900 dark:text-white rounded-xl border border-slate-200 dark:border-slate-700 text-xs focus:ring-2 focus:ring-brand-500/50 focus:outline-none">
            </div>
            <div>
              <label class="block text-xs font-bold text-slate-700 dark:text-slate-300 mb-1">Company Email *</label>
              <input type="email" [(ngModel)]="form.companyEmail" name="companyEmail" required placeholder="contact@apex.com" class="w-full px-3.5 py-2 bg-slate-50 dark:bg-slate-800 text-slate-900 dark:text-white rounded-xl border border-slate-200 dark:border-slate-700 text-xs focus:ring-2 focus:ring-brand-500/50 focus:outline-none">
            </div>
          </div>

          <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              <label class="block text-xs font-bold text-slate-700 dark:text-slate-300 mb-1">Person In Charge *</label>
              <input type="text" [(ngModel)]="form.personInCharge" name="personInCharge" required placeholder="John Doe" class="w-full px-3.5 py-2 bg-slate-50 dark:bg-slate-800 text-slate-900 dark:text-white rounded-xl border border-slate-200 dark:border-slate-700 text-xs focus:ring-2 focus:ring-brand-500/50 focus:outline-none">
            </div>
            <div>
              <label class="block text-xs font-bold text-slate-700 dark:text-slate-300 mb-1">Phone Number</label>
              <input type="text" [(ngModel)]="form.phone" name="phone" placeholder="+1 (555) 123-4567" class="w-full px-3.5 py-2 bg-slate-50 dark:bg-slate-800 text-slate-900 dark:text-white rounded-xl border border-slate-200 dark:border-slate-700 text-xs focus:ring-2 focus:ring-brand-500/50 focus:outline-none">
            </div>
          </div>

          <div>
            <label class="block text-xs font-bold text-slate-700 dark:text-slate-300 mb-1">Address</label>
            <input type="text" [(ngModel)]="form.address" name="address" placeholder="Factory 4, Industrial Zone..." class="w-full px-3.5 py-2 bg-slate-50 dark:bg-slate-800 text-slate-900 dark:text-white rounded-xl border border-slate-200 dark:border-slate-700 text-xs focus:ring-2 focus:ring-brand-500/50 focus:outline-none">
          </div>

          <div>
            <label class="block text-xs font-bold text-slate-700 dark:text-slate-300 mb-1">Notes / Instructions</label>
            <textarea [(ngModel)]="form.notes" name="notes" rows="2" placeholder="Special requirements or contact notes..." class="w-full px-3.5 py-2 bg-slate-50 dark:bg-slate-800 text-slate-900 dark:text-white rounded-xl border border-slate-200 dark:border-slate-700 text-xs focus:ring-2 focus:ring-brand-500/50 focus:outline-none"></textarea>
          </div>

          <div class="flex space-x-3 pt-2">
            <button type="button" (click)="closeModal()" class="flex-1 py-2.5 bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 font-bold text-xs rounded-xl">
              Cancel
            </button>
  ...
  saveStakeholder() {
    ...
    this.stakeholderService.createStakeholder(orgId, this.form).subscribe({
      next: () => {
        ...
        this.closeModal();
        this.loadStakeholders();
        this.form = { role: 'MANUFACTURER', companyName: '', companyEmail: '', personInCharge: '', phone: '', address: '', notes: '' };
      },
      error: (err) => {
        this.loading.set(false);
